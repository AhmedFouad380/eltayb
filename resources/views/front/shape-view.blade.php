@if($data->Product->is_discount == 'active')
    ...
    <span data-shape="{{$data->id}}"class="current-price text-brand">{{ $data->StorageAvaliable->sell_price - (( $data->StorageAvaliable->sell_price * $data->Product->discount_value ) / 100 ) }}</span>
    <span>
